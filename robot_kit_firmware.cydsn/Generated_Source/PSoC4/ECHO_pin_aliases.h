/*******************************************************************************
* File Name: ECHO_pin.h  
* Version 2.20
*
* Description:
*  This file contains the Alias definitions for Per-Pin APIs in cypins.h. 
*  Information on using these APIs can be found in the System Reference Guide.
*
* Note:
*
********************************************************************************
* Copyright 2008-2015, Cypress Semiconductor Corporation.  All rights reserved.
* You may use this file only in accordance with the license, terms, conditions, 
* disclaimers, and limitations in the end user license agreement accompanying 
* the software package with which this file was provided.
*******************************************************************************/

#if !defined(CY_PINS_ECHO_pin_ALIASES_H) /* Pins ECHO_pin_ALIASES_H */
#define CY_PINS_ECHO_pin_ALIASES_H

#include "cytypes.h"
#include "cyfitter.h"
#include "cypins.h"


/***************************************
*              Constants        
***************************************/
#define ECHO_pin_0			(ECHO_pin__0__PC)
#define ECHO_pin_0_PS		(ECHO_pin__0__PS)
#define ECHO_pin_0_PC		(ECHO_pin__0__PC)
#define ECHO_pin_0_DR		(ECHO_pin__0__DR)
#define ECHO_pin_0_SHIFT	(ECHO_pin__0__SHIFT)
#define ECHO_pin_0_INTR	((uint16)((uint16)0x0003u << (ECHO_pin__0__SHIFT*2u)))

#define ECHO_pin_INTR_ALL	 ((uint16)(ECHO_pin_0_INTR))


#endif /* End Pins ECHO_pin_ALIASES_H */


/* [] END OF FILE */
